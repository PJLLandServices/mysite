...
Quotes
  ...
  POST   /api/admin/smart-controller-quote       ← "+New Smart Controller Quote" (admin, zero-typing). Body {customerId, propertyId?, zoneCount?}; 422 codes property_required / zone_count_required drive the modal's progressive selectors. Auto-populates contact/address/zone count (system.zones.length → system.zoneCount), resolves the controller_* tier, mints lead + draft ai_repair_quote w/ narrativeKey:"smart-controller". 17+ zones → CRM lead only. Idempotent per property/customer. Manually-picked zone count saves back to property.system.zoneCount.
