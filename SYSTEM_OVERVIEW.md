...
Quotes
  ...
  POST   /api/quotes/:id/send-for-approval       ← admin "tap Send" on a draft ai_repair_quote (smart-controller upgrades): branded email w/ quote PDF + SMS, both pointing at the customer PORTAL (no approval token); flips quote draft→sent + lead→quoted so the portal Accept button activates. Re-send allowed while still "sent". on_site_quote uses /api/work-orders/:id/on-site-quote/send-for-approval instead.
