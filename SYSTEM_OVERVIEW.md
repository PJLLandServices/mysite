...
| `outreach.js` | — | n/a | Seasonal bulk-nudge engine (feature-seasonal-outreach-brief.md). Lists eligible properties per season+year (`listCandidates`), orchestrates per-recipient send through `notify-customer.js` with 300 ms Twilio + 100 ms Gmail pacing and a module-level concurrent-send lock (`sendBulk` → `{batchId, sent, skipped[], errors[]}`), derives "booked for season" state from bookings.json (`deriveBookingState`), validates and applies unsubscribe tokens (`honorUnsubscribe`), and persists per-season templates via `settings.js`. Season windows come from **`seasons.json`** at the repo root via `server/lib/seasons.js` (season-config brief, Aug 2026) — they used to be a hardcoded `SEASON_WINDOWS` constant in this file reading spring Mar 1 – Jun 30 / fall Sep 1 – **Dec 15**, which ran 39 days past safe frost and made `deriveBookingState` count Nov 7 – Dec 15 appointments as "booked for fall". **Fall 2026 is Sep 1 – Nov 6.** The move exists so the seasonal gate planned for `availability.js` reads one source rather than a second copy that drifts. Inclusive on both ends, local-date math on the `America/Toronto` process TZ, per-year: a year with no block in `seasons.json` inherits year-agnostic defaults, which end fall on the **Nov 6** frost stop rather than the old Dec 15 — an unplanned year inherits a safe date, not a known-wrong one, so fall 2027 cannot silently reacquire this bug. Spring's defaults are its long-standing Mar 1 – Jun 30. `seasons.json` also defines `publicBookingThrough` (fall 2026: Oct 30, reserving Nov 1–6 for admin placement) — **defined for the future gate, consumed by nothing today**, pinned by a source guard in `scripts/test-season-config.mjs`. `SEASONAL_SERVICE_PREFIXES` (`spring_open_`, `fall_close_`) stays hardcoded here. |
