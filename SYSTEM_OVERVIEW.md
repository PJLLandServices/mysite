...
| `billing-parties.js` | — | `con_xxxxxxxx` (contacts) | **Resolves WHO is billed and THROUGH WHOM** from the `(property, customer)` pair — the single source for the bill-to envelope, so invoices, quote PDFs and the admin UI can never disagree. `resolveBillTo(property, customer, {fallbackAddress})` → `{ name, careOf, address, email, managed }`. Two branches: **managed** (the property names its own `billingEntity` AND a differently-named customer exists to bill through → `name` = the site's entity, `careOf` = the managing company, `address` = the manager's billing address, `managed:true`) and **everything else** (residential, or a commercial account whose entity equals its own name → `name` = `billingEntity \|\| customer.billingName \|\| customer.name`, `careOf` = the manual `customer.commercial.careOf` override, `managed:false`). The second branch is **byte-identical to pre-commercial behaviour**, which is why residential invoices are untouched; a regression test pins the whole envelope object. `resolveContactRoles(property, customer)` → `{ signatories, siteContacts }`, each entry tagged `from: "customer" \| "property"` — customer contacts are org-wide, property contacts are site-scoped. Also owns **contact identity**: `isContactId` / `makeContactId` / `coerceContactId` mint the `con_` ids used by BOTH contact lists, so the two can never drift into different id formats. Consumed by `invoices.js` (draft `billTo` snapshot), `server.js` (quote send + admin), `quote-pdf.js`, and the property/customer admin pages. **Historical invoices keep their snapshotted envelope — a later `billingEntity` edit never retro-rewrites an issued invoice (Hard Rules 2 & 10).** |
...
customers.json             ← canonical Customer records (numeric QB-style ids; name/phone/email + spouse fields, billingName/billingEmail/billingAddress, status, quickbooksId, notificationPrefs, negotiatedRates, communicationRecords, history). Commercial layer: `accountType:"residential"|"commercial"` and `commercial { careOf, poRequired, paymentTerms, contacts:[{id:"con_…", name, role, email, phone, isSiteContact, isAuthorizedSignatory}] }` — null on residential accounts. Signatories are ORG-WIDE (they sign for every site the customer manages); per-site people live on the property instead.
properties.json            ← customer site profiles (now includes seasonalEligibility, seasonalOutreach[YYYY:season].{touches[], optOutThisSeason}, and commPrefs.{seasonalRemindersSMS, seasonalRemindersEmail, optOutTokens}; customerName is required non-blank). Commercial layer: `billingEntity` (this site's own legal payer, e.g. "YRSCC No. 1233 — Lewis Honey Condominiums"; empty = the customer is the payer) + `siteContacts[]` (same contact shape as `customer.commercial.contacts`, scoped to THIS site).
