...
UNMAPPED, needs a walked acceptance Patrick can't do from this sandbox (see the row for why).
...
| VD-13 | **Booking funnel admin dashboard** (2026-09-20, does not touch FLOW-03 or any other PASS flow — new admin-only read-only reporting surface). Built on top of VD-12's `booking_step` GA4 events: `server/lib/ga4.js` (new) authenticates as a Google Cloud service account (Viewer-only on the GA4 property; JSON key in the `GA4_SERVICE_ACCOUNT_KEY` Render env var, numeric property in `GA4_PROPERTY_ID`; both set by Patrick, neither committed) and calls the GA4 Data API's `runReport` directly via `fetch` — no new npm dependency, matching this repo's existing raw-HTTP convention for external Google APIs (`lib/geocode.js`, `lib/distance.js`). `GET /api/admin/booking-funnel?days=7\|14\|30` (staff-gated via `needsAuth`, same as every other `/api/admin/*` route) shapes three parallel GA4 queries into `{steps, byLocation, byService}`; results are disk-cached 20 minutes (`server/data/booking-funnel-cache.json`, gitignored under the existing `server/data/*` wildcard) to stay off GA4's query quota. New page `/admin/booking-funnel` (`server/booking-funnel.html`, nav link added to all 40 existing admin pages) renders it with a date-range selector and three empty/error/data states. **Fail-soft, three layers deep, same posture as `geocode.js`'s town-centroid fallback:** (1) no credential/property ID → clean `ok:false, reason:"not_configured"`, never a throw; (2) a GA4 fetch failure with a still-good cache on disk → serves the cached numbers marked `stale:true` rather than blanking the page; (3) a fetch failure with no prior cache → clean `ok:false, reason:"fetch_failed"` carrying Google's own error text, never a stack trace to the browser. **Verified:** `scripts/test-booking-funnel.mjs` (19 assertions, wired into `build:check`) — all three fail-soft paths, correct step ordering (GA4 returns rows in its own order, re-sorted here to the visitor's actual flow order), the location `(not set)` → "Unknown" relabel, the deep-linked-visitor split per service, and that a cold-cache in-flight token fetch is de-duplicated across the three parallel report calls (initially wasn't — three token round-trips on every cache-cold window instead of one — fixed before merge). `node --check` on both new/touched server files, `node build.js --check`, `node scripts/lint-booking-links.mjs`, and a real (extracted, executed) `needsAuth()` check confirming both the page and API paths resolve to `"user"` (staff-gated) and an unrelated route is untouched. | **Not verified — needs Patrick, and is why this isn't marked done:** the real GA4 Data API round-trip. Two things stand between this and real data: (1) `GA4_PROPERTY_ID` — Patrick has not yet provided it; (2) `step_name`, `service_key`, and `deep_linked` (the `booking_step` event params this report reads) are **not yet registered as GA4 custom dimensions** — until Patrick does that in GA4 Admin → Custom definitions (Event scope, one per param), the Data API will reject the report with an "invalid dimension" error, which surfaces as the ordinary `fetch_failed` fail-soft path, not a crash — but it means no real numbers until both are done. Tracked as PJL-38/PJL-39/PJL-40 in Linear (project: Booking Funnel Analytics Dashboard). | Medium |
| VD-12 | **Booking funnel step tracking — confirm live** (2026-09-17, touches FLOW-03 only). Ads spend was recording 0 conversions (123 impr / 9 clicks / $57.43 / 7 days, 5 of 8 clicks to `book.html?service=fall_close_4z`) with no visibility into which of the 5-or-6 steps (repair/retrofit/site-visit skip ZONES) a visitor actually reached. Added a GA4 `booking_step` event, fired once per `showStep()` render plus once for the two `init()` landings that show "service" without calling `showStep()` (the plain default landing and a multi-variant `?service=` deep link) — every landing on the page fires exactly one event, covered by both cases. Fields: `step_name`, `step_number` (1-based, computed from the current visitor's actual order — zones included or not), `service_key` (`state.serviceKey`, `"(none)"` before a service is chosen), `deep_linked` (set once from `?service=` presence, carried on every event this session), `is_repeat_view` (true on a second render of the same step name — back-nav or re-render, so one `showStep()` call still produces exactly one event and a repeat is visible in the data rather than silently duplicated). `js/booking.js` bumped `?v=6` → `?v=7` in `book.html` (the only file referencing it — not a partial). **Nothing else touched:** the existing `gtag('event','conversion', …)` calls (Website Phone Click, Booking Request) are unmoved, unwrapped, and unconditional on the new code; no route, payload, step order, or step count changed. **Verified automatically** in headless Chromium (stubbed `/api/booking/services` + `/api/booking/availability`, read `window.dataLayer` directly since `gtag.js` is unreachable from this sandbox): default landing → exactly one `service` event, `step_number:1`, `deep_linked:false`, `service_key:"(none)"`; `?service=fall_close_4z` deep link (multi-variant family, so `init()` never calls `showStep()`) → exactly one `service` event, `deep_linked:true`, `service_key:"fall_close_4z"`; click-through service → zones → back-to-service → `zones` event carries `step_number:2` and the chosen `service_key`, and the second `service` view is flagged `is_repeat_view:true` rather than firing unflagged. `node build.js --check` and `node scripts/lint-booking-links.mjs` both pass. **Not verified:** the actual GA4/Ads dashboard ingest — same sandbox limitation as VD-10. | Patrick to confirm on the live site: DevTools → Network (filter `collect` or `google-analytics`) shows a `booking_step` hit with `step_name=service` on load of `book.html?service=fall_close_4z`, then one more per step clicked through. Confirm page source now loads `booking.js?v=7`. Do **not** complete a real test booking to check this — it fires the live Ads conversion and creates a real booking. Two weeks of paid traffic after this ships should answer: of N people who landed on the booking page, how many reached PICK A TIME. **FLOW-03 stays PASS — this change is additive, client-side only, and does not touch the hop chain a walked booking exercises.** | Small |
...
| FLOW-43 | **A self-booked customer's minute follows the route** — **UNMAPPED** (opened 2026-09-12) | Hop chain: **any plan change (window, order, auto-order, move, add, place, day move) or any booking (every path runs `syncBookingFromLead` → `restampDayInDrivingOrder`) or the 10-minute sweep → `syncRoutedTimes(season, year)` → `gatherBookedRows()` (now carrying each booking's duration) → `assignments.syncAssignedTimes(…, { bookedRowsByDate, retimeBooking })` → ONE sequence per date via `assignments.sequenceWithBookings` (plan stops + booked customers, moved from server.js) → assigned bookings re-anchored as before, now WITH the customers in the loop → each customer's placeholder moved to its sequenced arrival, clamped by `scheduledStartFor` inside the half-day they were told → `retimeCustomerBooking()` moves the lead's embedded booking and the bookings record together through `mirrorBookingOnly` (the one mirror) plus any linked work order, writes a "Route timing: 10:30 → 08:20 (arrival window unchanged)" line on the customer's record, sends nothing**. **TIME-01 (found + fixed 2026-09-12):** Patrick, on Cynthia Tam's row reading 10:30 a.m. as stop 1 of a morning the route reached at 08:20: "instead of the time slot being a place holder, can that also move around as well? The only place holder that is contained is the 8–12, and 12–5 ... Keep just the two time slots." Two things were true: (a) the online booking's minute was the engine's first free 30-minute mark, held forever, so the field app said 10:30 while the route said 08:20; (b) `syncAssignedTimes` timed the assigned stops from a sequence that did NOT include the self-booked customers on the day, so the board (which had them) and the calendar (which didn't) printed different minutes for the same stop. A third thing was lurking: `restampDayInDrivingOrder` (2026-09-09) was a second, lead-only re-timing implementation — 30-minute grid from the top of the half-day, refusing any assigned day — that could not see plan stops and silently did nothing for a lone self-booked customer on a route day ("fewer than two lead bookings"). **One rule.** The half-day is the promise; the minute inside it is the route's. `sequenceWithBookings` is the single day sequencer (board, route line, preview, time sync); `syncAssignedTimes` times assigned AND self-booked from it; the re-stamp delegates to the same sync. Never moved out of its half-day; never on a day already driven; never a visit the tech has arrived at or a locked day; never a customer with no pin; nothing sent. A season with no plan still times its booked days from the yard. **Requested windows stay assigned-customers-only** (their appointment page), as Patrick asked: online customers pick a half-day and nothing finer. **Deliberately left alone:** `orderDayForDriving` in `/api/schedule/today` (the safety net for days whose stamps predate this); the 30-minute grid the booking page offers slots on; the double-booking guard, which still compares minutes (a moved placeholder frees the mark it left). `scripts/test-routed-times.mjs` (25, in `build:check`) pins the placeholder following the route, the assigned stops timed with the customer in the loop, both half-day clamps, the four leave-alones, the no-plan season, and the one-sequencer / one-mirror / delegating-re-stamp wiring; **verified against the unfixed code first.** The status-reader lint's pinned re-stamp line moved with the code. **Patrick's acceptance test — not yet walked:** (1) after deploy, within ten minutes Cynthia Tam's row reads 08:20 and the field app agrees; her customer record shows the "Route timing" line; her window on the confirmation is unchanged; (2) move a stop on that day — every time on the day, hers included, re-cuts; (3) a new online booking lands at its routed minute, not 08:00 or the first free mark; (4) nobody receives a message about any of it. |
| FLOW-42 | **A moved stop takes its appointment with it** — **UNMAPPED** (opened 2026-09-12, the night of the blast) | Hop chain: **"Move to…" on a stop → `PATCH …/move` → `seasonPlans.moveStop()` saves the plan → `assignments.followPlanMoves(season, year, { codes: [code] })` → the property's confirmed assignment booking, if the customer has not moved it themselves, goes to the plan's new date and half via `bookings.moveAssignmentDay()` (now takes `toBucket`): re-timed from the day's sequence, response reset, day-move notice queued when they had the blast → the toast says which of those happened**. For stops moved BEFORE this existed: **the board's `planMoves` (a dry run of the same function) → the amber banner "N appointments are still on the day their stop was moved FROM" → `POST …/follow-plan` (armed twice) → the same moves**. **MOVE-01 (found + fixed 2026-09-12):** Patrick, after moving a batch of stops: "tell me though is this automatically saving and keeping these dates now??" and, a minute later, "the app is showing accurate appointment info, but the web browser isn't." The plan file saved. The appointments did not move: a single stop's move touched the stored plan only, while a whole-day move (`moveDayBookings`, FLOW-3x stage 6) had always carried its bookings along. The field app reads bookings, so it was right; the desktop Season Plan read the plan, so it showed the stop on the new day while the calendar, the day sheet and the customer all still had the old one — and once FLOW-40 landed, the driven plan judged those stops "moved" back to where their bookings were. Two writers of one promise, one of them silent. **Rule, once:** `followPlanMoves` is the single-stop counterpart of `moveDayBookings`, by its rules — confirmed assignment bookings only; the customer's own reschedule (`scheduledFor` no longer on `assignment.date`) is theirs to keep and is left alone (FLOW-40 draws it on the day they chose); a free-bucket customer moves without a notice or a reset; the notice is queued for the cadence sweep, never sent here; a same-day half-day change is a move too (the notice already names the half). `codes` limits it to the stops just moved; `dryRun` reports. **Deliberately left alone:** the whole-day move (its own ride-along, unchanged); `preflight`/`assign`; the stored plan as the record. `scripts/test-plan-move-follows.mjs` (29, in `build:check`) pins each rule on fixtures, the record keeping the half-day, the route wiring, the dry run, and the screen's words; **verified against the unfixed code first.** **Patrick's acceptance test — not yet walked:** (1) after deploy, open Season Plan: an amber banner counts the appointments still on the day their stop was moved from; press **Move N appointments to match** twice; the banner goes, each moved stop now reads as a normal numbered stop on its new day, and the field app agrees; (2) the customers who had the blast get "was <old>, now <new>" at the next 9 AM send (Assignment drawer → cadence status shows the owed notices); (3) move one more stop with Move to… — the toast says "Appointment moved with it — the customer will be told at the next send"; (4) a customer who rescheduled themselves is never pulled back. |
| FLOW-41 | **Anything waiting for a day is dragged onto one** — **UNMAPPED** (opened 2026-09-12) | Hop chain: **`loadUnplanned()` / `loadStandby()` → `renderTray()` draws a chip per waiting property and per open-bucket customer on the cockpit's bottom tray → drag → every rail row shows `+N min` for that chip (`annotateRailForDrag`, best in green) → drop on a rail day (`wireDropTarget(row)` → the lighter half-day by `lighterBucketOf`, the server's `lighterBucket` rule read from the same counts) or on a half-day block (`wireDropTarget(wrap)`) → `dropRefusal()` says no (yesterday, a booked-only day for a plan stop, the morning or an unpinpointed address for an open-bucket customer) or → property: `addToDay()` (the one `/add` caller; nothing sent until Assign) · open-bucket: `bookStandby()` (slot resolver → reserve → confirmation sent), the SAME function the drawer's button now calls**. **TRAY-01 (2026-09-12):** Patrick: "can the 'not on the plan' be somewhere on this page and I can just drag and drop them into a day.. There's too many avenues that you have to go down when choosing this. It should be the same for the 'Open Bucket' as well." Both lists were behind toolbar drawers, each with its own picker (FLOW-37's day select, FLOW-39's preview window, the bucket's select + Book). Now the cockpit has a fourth region under the map: chips for everything waiting, dragged onto the day. The routing answer rides on the rail while a chip is in the air rather than in a window. A chip click still opens FLOW-39's preview for a property that he wants to look at first. The drawers stay: HTML5 drag-and-drop does not work on touch screens, and the bucket drawer's button and a drop share one `bookStandby`, so they cannot drift. **Deliberately left alone:** the server — every drop lands on routes that already exist (`/add`, `/api/admin/open-bucket/slot`, `/api/booking/reserve`); *Place all on their best days* for the lot. `scripts/test-place-tray.mjs` (34, in `build:check`) pins the half-day rule, the drag payload, every refusal, the one-add-path and one-book-path wiring, and that the drawer's button and the drop share the function; **verified against the unfixed page first.** **Patrick's acceptance test — not yet walked:** (1) open Season Plan; a tray under the map lists Not on the plan and Open bucket chips with their best day and cost; (2) drag a property chip: every day in the rail shows `+N min`, the best one green; drop it on a day — it lands in the lighter half, the board jumps there with the stop flashed, the chip leaves the tray; (3) drop one on a Morning block — it lands in that morning; (4) drag an open-bucket chip onto a Morning block — red outline and a toast saying afternoons only; drop it on the day — booked into the afternoon, confirmation sent, chip gone; (5) drop anything on a past day — refused. |
| FLOW-40 | **A cancelled or moved stop leaves its route day** — **UNMAPPED** (opened 2026-09-12, the morning after the blast) | Hop chain: **customer cancels / no-shows / reschedules → `bookings.cancel()` or `bookings.reschedule()` flips the booking → every plan reader calls `assignments.drivenPlan(season, year)` → `planStopState(code, date)` judges each stored code by its property's bookings → `planAsDriven()` removes the gone codes from their day and lists them in `gone[date]` → the board draws the day without them and prints "Off this day: 12 Oak St · Alfie Muzzin — customer cancelled — Selling the house" on the day's notes strip**. **PLAN-01 (found + fixed 2026-09-12):** Patrick: "there's some appointments that were cancelled when we sent out the appointments this morning. They are not reflecting on the map. Also there are properties that we don't go to, they are still on the route. Can this route not be updating like it was supposed to be? ... am I not thinking this through correctly?" He was thinking it through correctly. The plan is a stored list of property codes per day; Assign turns each into a booking and from then on the booking is the truth — but no code path ever removed a code from the stored plan (there is no `removeStop`), and every reader took the stored codes as the route. A cancelled stop kept its number, its pin and its share of the drive; a customer who moved to Thursday was drawn on Tuesday AND Thursday (`gatherBookedRows` puts the moved booking on its new date as a booked row, the stored code stayed on the old one). The FLOW-33 lesson (`bookingHoldsItsSlot`, two passes) in a new coat: the state flipped, one reader honoured it, the rest did not. **One rule, derived, never written.** `planStopState({date, bookingsForProperty, season, year})` → `on_day` (a live booking for the property IS on this day, any source — a hand re-book after a cancellation counts), `done` (completed on this day; history, kept), `unassigned` (no assignment booking yet; the plan is still the intent), `moved` (the assignment lives on, on another day — drawn THERE as a booked stop), `cancelled` / `no_show` (dead, and nothing else on this day; carries the customer's reason, code and time). `stopIsGone()` = moved/cancelled/no_show. `planAsDriven(plan, stateFor)` is pure and leaves the stored plan alone: the stored plan is the audit trail, and a customer who changes their mind can be put back. **Every reader, walked:** the board (`resolveSeasonPlan`, each day gets `dropped`), the day preview (FLOW-39; guards "already on the plan" against the STORED plan), the route line, the route-map image, `dayShapesForSeason` (the geography gate customers are offered days by — a cancelled stop must not gate a day it left), `unplannedRanker`, the phone probe, the job finder (search), `syncAssignedTimes` (a dead stop must not lend its minutes to the remaining stops' calendar times), and the place-on-best bucket choice. **Deliberately left alone:** the write paths (`/add`, `/move`, `/day`, auto-order, place) read and save the STORED plan; `moveDayBookings` reads the stored plan because it runs after the day moved and before its bookings have — the driven day would be empty at that instant; `preflight`/`assign` already skip cancelled (`assignment_declined`) and moved (`previously_assigned`) properties; the cadence engine selects by booking status, never by plan. **What the customer sees:** nothing new — their appointment page already said cancelled/moved. **What Patrick receives:** the day's notes strip names each stop that left and why, in grey strike-through, not red — it is the record, not a problem. **Capacity:** counts, caps, the lighter-bucket rule and the day shapes all exclude gone stops. `scripts/test-plan-follows-bookings.mjs` (39, in `build:check`) pins every state of the rule, the pure driven plan, `drivenPlan` on fixtures, that each reader named above holds the driven plan (and the two that must not, don't), and the screen's words; **verified against the unfixed code first.** Two superseded source assertions in `test-unplanned-routing` and `test-day-map-preview` updated. **Patrick's acceptance test — not yet walked:** (1) open Season Plan on a day that had a cancellation this morning; the cancelled stop is gone from the numbered list and the map, the count is one lower, and the notes strip reads "Off this day: <street> · <name> — customer cancelled — <their reason>"; (2) a customer who rescheduled to another day appears ONLY on the new day, as a booked (amber) stop, and the old day's strip says "moved to <day>"; (3) Open route / the route line no longer passes that driveway; (4) Not on the plan → See it on a day: the drive figures no longer include the cancelled stop. |
| FLOW-39 | **An unplanned property is SEEN on a day before it is added** — **UNMAPPED** (opened 2026-09-12, the morning after FLOW-37 went live) | Hop chain: **row → "See it on the best day" (or any day from "Look at a day…" / "Any other date…") → `GET …/season-plans/:season/:year/preview/:date?code=&bucket=` → `resolvePlanDay()` for the day as stored AND for `dayPreview.planDayWithCandidate()` (the stored day plus this code in one bucket) → `routeGeometry.roadLine()` through `dayPreview.planLineStops(after)` → the dialog draws that day with `drawDayMap(…, { line })`, the candidate pin lit amber, and prints `before → after` drive minutes, home time and stop count → he presses **Add to this day** (the ONE write, `POST …/add`) or **Not this day**; after an add the board jumps to that day with the stop flashed**. **ROUTE-02 (found + fixed 2026-09-12):** FLOW-37 answered the routing question with a number — "Best: Oct 6 · +4 min" — and a blind per-row *Place on best day*. Patrick: "honestly, that's literally no help either. Can you do something that allows me to see the map of what the day would look like with the appointment incorporated, and I choose whether or not I want to add it to that day?" A number is not a route; the day he would actually drive, with the new stop numbered into it and the road line drawn through it, is what he can judge. **The preview is built by the board's own day builder.** `resolveSeasonPlan()`'s per-day body was lifted into `resolvePlanDay()` (with `resolvePlanStop` and `numberBookedRows` beside it) and both the board and the preview call it, so the previewed day is byte-for-byte what the board will show once he says yes — a second builder would drift exactly when it mattered (the FLOW-36/Today-map lesson, `day-preview.js`). The stored day is carried across untouched (other bucket, label, time windows, manual order); the candidate is appended to the asked-for bucket or `lighterBucket()`; a code already on the plan is a 409, not a doubled stop; a property Google can't place is a 422 that says so rather than a map with the expensive stop invisible. The route reads and writes nothing. `numberBookedRows` mutates the rows it is given, so each side gets its own copies — "before" cannot inherit "after"'s numbers. **On the page**, the per-row blind *Place on best day* is gone; the picker's every option now opens the preview for that day instead of adding; `addToDay()` is the single `/add` caller; `drawDayMap` takes a caller-supplied `line` (a day that isn't stored can't be asked for by date) and `paintLine` is the one line painter for both; `pinIcon` lights `stop.candidate` with the same amber ring a hovered stop gets, so there is no new colour to learn. A stale answer for a day he has already moved on from is dropped by sequence number. **Deliberately left alone:** *Place N on their best days* for the lot still places by rule, armed twice — bulk is bulk; the day rail and the stored day's map are unchanged. `scripts/test-day-map-preview.mjs` (49, in `build:check`) pins the hypothetical-day rule, the summaries and delta, timeline-ordered line stops, the one-builder wiring, the read-only route and its guards, and the page flow; **verified against the unfixed code first** (49 fail, none crash). `test-unplanned-routing.mjs`'s two superseded assertions were updated to the new button. **Patrick's acceptance test — not yet walked:** (1) Season Plan → Not on the plan → **See it on the best day** on an Alfie Muzzin row; a window opens with that day's map, the new stop numbered and ringed amber, the line through it, and chips reading *Driving: 70 → 84 min (+14)*, *Home: … → …*, *N → N+1 stops*; (2) switch the Day select to another day — the map and chips redraw for that day; switch Half — the stop moves bucket and the numbers change; (3) **Not this day** closes with nothing added; (4) **Add to this day** — the board jumps to that day with the stop flashed and the row leaves the drawer; (5) run Assign — exactly those bookings are created. |
| FLOW-38 | **A deploy reaches the browser** — **UNMAPPED** (opened 2026-09-12, after the third "it's not doing anything" of the day) | Hop chain: **deploy → `ASSET_VERSION` (Render's `RENDER_GIT_COMMIT`, else boot time) → `stampAssetVersions()` on every CRM `.html` the static sender serves → `<script src="/crm/x.js?v=…">` → the browser fetches a URL it has never seen**. **CACHE-01 (found 2026-09-11 morning, fixed 2026-09-12):** the server sends `cache-control: no-cache` for `/crm/*.js` and `*.css` — the correct header, added months ago for exactly this class of bug ("ran old JS against new HTML"). **Cloudflare's Browser Cache TTL overrides it to `max-age=14400` on the wire.** Measured directly: `curl -D - /crm/season-plan.js` → `cache-control: max-age=14400`, four hours, while the origin says `no-cache`. So a hard refresh made BEFORE a deploy finished pinned the previous file for the afternoon, and three separate deploys that day (#207, #208, #210) each produced a round of Patrick being told what the screen should show while his browser ran yesterday's `season-plan.js`. The catch-up button, the resend list and the routed unplanned drawer were all live on the server and invisible to him. **The durable fix does not depend on anyone's CDN settings:** every CRM script and stylesheet URL in served HTML now carries `?v=<deploy stamp>`, so a deploy is a new URL and no cache anywhere can hand back the old file. The HTML page itself was already `no-store` and stays so. `tech-sw.js` is deliberately **not** stamped — a service worker's registration URL must not change between loads or the browser installs a second worker. Already-queried URLs, images, the public site and inline mentions are untouched. `scripts/test-asset-versioning.mjs` (in `build:check`) pins the rule against strings and proves it on a booted server via `/login`, the one CRM page that needs no session: script and stylesheet stamped with the commit, logo untouched, page still `no-store`, `content-length` equal to the stamped body, stamp stable across requests, the versioned asset URL still served. **Still worth doing on Cloudflare** (Caching → Configuration → Browser Cache TTL → *Respect Existing Headers*): the stamp makes stale JS impossible after a deploy, but the CDN override remains wrong for everything else it touches. **Patrick's acceptance test — not yet walked:** (1) after this deploys, view-source on `/admin/season-plan` and confirm `/crm/season-plan.js?v=` with a 10-character stamp; (2) confirm the page behaves as the latest PR describes WITHOUT a hard refresh; (3) confirm the tech field app still installs its offline worker (the one URL that must stay unversioned). |
| FLOW-37 | **An unplanned property is placed by drive time, not by hand** — **UNMAPPED** (opened 2026-09-11, the evening the first cut of FLOW-36 went live) | Hop chain: **`GET …/unplanned` → `unplannedRanker()` builds the plan's day shapes once → `geoFilter.rankDaysForPoint(coords, shapes)` per property → rows carry `days` (cheapest first) and `best` → the drawer shows "Best: <day> · +N min" and a cost-ordered picker → `POST …/unplanned/place` → `best.date` + `assignments.lighterBucket()` → `seasonPlans.addStop()` per stop → one resequence**. **ROUTE-01 (found + fixed 2026-09-11):** the FLOW-36 drawer offered a bare "Put on…" day-picker with no drive times. Patrick, on seeing nineteen real properties listed that way: "the only option it has for me is for me to choose the day that it sits on. But it doesn't show me routing etc? our entire implementation of this is so that we can efficiently have driving routes that make sense." He was right — a picker with no costs asks him to route by hand, the one job the system exists to do. **I had also misread those nineteen as test data from a screenshot; they are real properties.** Recorded here so the next person doesn't repeat it: a batch of same-customer properties with sequential codes is what a property manager's portfolio looks like, not proof of a seed script. Fixed with **one named rule**, `rankDaysForPoint`, which answers for a property exactly what the probe answers for a typed address — cheapest-insertion added drive against every route day, sorted, with `offered`/`widensAtMinutes` against the same `geoMaxAddedDriveMinutes` threshold and `GEO_WIDEN_TIERS` the booking page uses — and picks `best` as the cheapest OFFERED day that has stops (an empty day at +0 is a blank calendar, not "we're nearby"). Yesterday is never listed. The depot fallback coordinate is refused as a ranking point (it would make every day look cheap). A property with no stored coordinates is geocoded through `geocodeForRecord`, the disk-cached path the importer uses, so re-opening the drawer costs nothing. Day shapes are built **once per request**, not per property. The list carries the ranking on each row; a row that can't be ranked is **shown and says why** (no coordinates, or the ranker failed) rather than guessed at. The picker's options are in routing order, each labelled with its cost. **`POST …/unplanned/place`** (`codes` or `all: true`) places each rankable row on its `best.date`, in the lighter bucket (`lighterBucket`, morning on a tie), through `addStop` — the one write path — re-reading the plan per stop so two properties choosing the same day see each other, resequencing once at the end, and reporting `unrankable` / `no_offered_day` per code instead of placing anything it couldn't rank. The drawer gains **Place on best day** per row and **Place N on their best days** for the lot, armed twice like every send on that page. **Deliberately left alone:** the hand-picked day still works and still wins — the ranking advises, Patrick decides. `scripts/test-unplanned-routing.mjs` (in `build:check`) pins the sort, the best-day rule, the empty-day and depot exclusions, the allowance and widening tiers, unrankable-is-said-not-guessed, the bucket rule, the ranker-failure isolation, and the wiring; **verified against the unfixed code first.** **Patrick's acceptance test — not yet walked:** (1) open Season Plan → Not on the plan; every row should read "Best: <day> · +N min · N stops" in green, and the picker's first options should be the cheap days; (2) a Primrose Lane property should rank the Newmarket day at a few minutes and an Oshawa day at an hour; (3) *superseded 2026-09-12 by FLOW-39* — the per-row button is now **See it on the best day**, and the add happens from that window; (4) press **Place N on their best days** twice and confirm the count; (5) run Assign — exactly those bookings are created. |
| FLOW-36 | **A property added after the plan import reaches a route day** — **UNMAPPED** (opened 2026-09-11) | Hop chain: **property created in the CRM → `assignments.unplanned()` (eligibility via `outreach.assessEligibility`, the same gauntlet preflight and assign run; prior-assignment rule via `priorAssignmentsFor`) → `GET /api/season-plans/:season/:year/unplanned` → the Season Plan's **Not on the plan** toolbar button + drawer → day picker → `POST /api/season-plans/:season/:year/add` → `seasonPlans.addStop()` → resequence → Assign books it**. **PLAN-01 (found + fixed 2026-09-11):** Patrick — "Additional properties that I am adding aren't backfilling on the Season Plan." **They couldn't, and nothing said so.** The plan is a once-a-season pasted import (`PUT /api/season-plans/:season/:year` REPLACES it), and the page's own design note said the only edit worth having was moving a stop between days because "the plan is regenerated each season". Assign books only codes already on a day. So a property created AFTER the import had no way onto a route day and — the real defect — no way to be **noticed**: not skipped, not a problem in the Needs-attention drawer, not a warning. It simply did not exist to the plan. That assumption held for a one-shot season and broke the moment a live one was being run with customers added as it went. **Not a regression** — never built. Fixed with the one addition that reasoning left out: `addStop()` is the mirror of `moveStop()` (same validation, same "any calendar date grows a new day" rule) and **refuses a code already in the plan** — that is a move, and one operation per state is how the two never disagree about what happened. `plannedCodes()` is the single answer to "is it on the plan at all?", read by both the unplanned list and addStop's refusal. `unplanned()` returns **two lists on purpose**: `placeable` gets a day picker; `blocked` is everything eligible-looking that can't be placed, **with its reason** (`season_opt_out`, `assignment_declined`, `previously_assigned`, `missing_name`, `no_code`…), because "not on the list" is exactly the silence this exists to end. A customer who booked their own appointment is settled and appears in neither. **PLAN-02 (same change, hygiene):** assign()'s "once ever, whatever became of it" prior-assignment rule was inline; it is now the named `priorAssignmentsFor()` that assign() refuses on and unplanned() reports on — two copies would eventually disagree about the same customer. The toolbar button wears the count and is **loud** (`is-warned`, like Needs attention) so a new property is visible without opening anything; the add toast says "Run Assign to book it" because adding to the plan is not booking. `scripts/test-season-plan-unplanned.mjs` (38 assertions, in `build:check`, sandboxed plan file like test-season-plan-moves) pins addStop's four refusals, the grown-day case, both lists with every blocked reason, the shared rule, the routes, the resequence, and that the drawer names every reason in plain words rather than raw snake_case. **Verified against the unfixed code first: 34 of the 38 fail.** All four assign()-exercising suites (writer 46, preflight 31, messages 77, appointment-page 50) re-run green after the extraction. Routes confirmed on a booted server (401 unauthenticated). **Patrick's acceptance test — not yet walked:** (1) add a property in the CRM with a zone count and fall eligibility, open the Season Plan, and confirm a red **Not on the plan · 1** button appears in the toolbar; (2) open it, pick a day and bucket, and confirm the toast, the day card gaining the stop, and the button count dropping; (3) run Assign and confirm exactly that one new booking is created and every existing stop reads *previously assigned*; (4) opt a second new property out of fall and confirm it appears under **Can't be placed** with "opted out of this season" rather than vanishing. **Deliberately not built, named:** auto-placement by geography — the probe drawer already answers "which day is cheapest for this address" and Patrick picks; wiring the two together is a later decision, not a silent default. |
| FLOW-35 | **A cadence step that never delivered gets sent** — **UNMAPPED** (opened 2026-09-11, closing the first gap FLOW-34 named as still open) | Hop chain: **`sendStepForBooking` marks `steps[n]` → sends → writes `{attempted, sent, errors}` → `channelsOwed()` reads the gap → `owedForBooking()` → `status().summary.owed` → the Season Plan button → `POST /api/assignments/:season/:year/catch-up` → `catchUpOwed()` → `sendStepForBooking(..., { only })`**. **CADENCE-01 (found + fixed 2026-09-11, the same outage as FLOW-34):** when the app password died mid-campaign the cadence marked its steps and **every email failed on authentication while the texts went out normally** — Twilio is a different provider. Each of those customers is on record as having had their step, with `sent: ["sms"]` and an email error beside it, and the sweep skips any step that is already marked (rule 1). The email was owed, **knowable from the step record**, and nothing was ever going to send it. Patrick: "I need to send the other emails too." **The delicate part is that a step record has THREE states and only one is owed:** no record (never fired — the sweep's own rules own it); a record with no `sent` key (**the crash window** — marked, result never written, the wire may have been touched: retrying here hands back exactly the double-send that marking first exists to prevent, so it is **never** owed); a record with `sent` (we know what went and what didn't — the difference is owed). `channelsOwed()` is defined **once** and both the count Patrick reads (`summary.owed`) and the send that acts on it call it, so they can never answer differently. A catch-up re-sends **only the missing channel** — the customer who got the text does not get it twice — does **not** re-mark the step (the record it would overwrite is the evidence the send depends on), merges the new channel into `sent` rather than replacing it, keeps the step's ORIGINAL `at`, and stamps `caughtUpAt` so it is never passed off as the original. **Deliberately NOT folded into the sweep, and pinned by a source guard:** rule 1 stays exactly as it was, because a sweep that retried owed steps by itself would re-send to a whole campaign the moment mail came back, with nobody deciding that. Patrick presses it, twice, like every other send on that panel. Every gate still runs — send window, send lock, `cadenceGates` (season opt-out, no-contact-needed) and per-channel capability — so somebody who opted out **between** the failed send and the catch-up is not caught up. A `GET` on the same route is a dry run naming who is owed what without sending anything. **CADENCE-02 (found + fixed in the same change, separate defect):** `sendOutreachEmail` logged to the ledger with **no `refId`**, so an outreach failure was an email address and nothing else — not enough to trace back to whose message it was. Both cadence callers now pass `booking.id`. **CADENCE-03 (same change):** a resend was indistinguishable from a first send in the ledger; entries now carry `resendOf` naming the attempt being made good, so "why did this customer get two?" has an answer in the only place that keeps the history. `scripts/test-cadence-catch-up.mjs` (40 assertions, in `build:check`) pins all three step states, the only-what-is-owed send, the merge, the gates, the dry run, and — with a source guard — that the sweep's rule 1 and the blast's already-blasted skip have not moved. **Verified against the unfixed code first: 34 of the 40 fail.** **A real bug this suite caught before it shipped:** the channel-narrowing edit also matched an identical line in `sendDayMoveForBooking`, which has no narrowing — `test-assignment-cadence` failed with `ReferenceError: channels is not defined`, i.e. every day-move notice would have crashed. Route existence and gating confirmed against a booted server (401 on GET and POST unauthenticated). **Patrick's acceptance test — not yet walked:** (1) with mail working, open the Season Plan and confirm the cadence line reads "N never went out" and a red **Send the messages that never went out** button appears; (2) press it twice and confirm the count sent matches; (3) confirm a customer who already had the text does **not** receive a second text; (4) reload and confirm the button is gone and the count is zero; (5) confirm Admin → Email health shows those sends as successful. |
| FLOW-34 | **An email that never went out gets sent** — **UNMAPPED** (opened 2026-09-11, the morning of the app-password outage) | Hop chain: **every sender → `mailerLog.logSend()` → `server/data/email-log.json` → `outstandingFailures()` → `GET /api/admin/email-health` → the admin panel worklist → `POST /api/admin/email-health/resend` → `EMAIL_RESENDERS[kind]` → the original sender, which logs its own retry**. **MAIL-01 (the incident, 2026-09-11):** Patrick changed his Google account password. Google revokes **every app password on the account** when you do that, and every outbound email in this system shares one pair (`GMAIL_USER` + `GMAIL_APP_PASSWORD`). Between ~08:00 and 09:04 the credential went dead and **56 sends failed on `535-5.7.8 Username and Password not accepted` inside an hour** — 50 outreach, 3 lead alerts, 2 customer cancellation confirmations, 1 other. Not a delivery failure: the mail never left the building. Diagnosed from the Email health panel plus the code; the Gmail connector needed re-authorising (same password change) so the Google security alert could not be read to confirm. **Not caused by the test-recipient gate** (`guardTransport` only intercepts `sendMail` for `PJLTEST-` recipients and never touches auth; the failing addresses were real customer domains). **MAIL-02 (found + fixed the same day):** nothing retries and nothing queues, so those 56 messages were simply gone — the ledger was the only evidence they had ever been attempted, and putting them right meant finding each customer by hand. Fixed with `outstandingFailures()` in `lib/mailer-log.js` and a resend path. Three rules, each pinned: **(a) outstanding ≠ failed** — a failure with a later success for the same kind+refId+recipient drops off the worklist by itself, and five attempts at one message are one row, so a resend can never mail a second copy of something already received; **(b) the message is REBUILT, never replayed** — the ledger keeps the fact of an attempt and never its body, so each kind regenerates from the record that still exists, and `booking_cancel` **refuses** when `bookingHoldsItsSlot()` says the booking came back to life (telling a live customer they were cancelled is worse than the silence being fixed); **(c) not everything may be resent** — `RESENDABLE_KINDS` is `booking_cancel` + `lead_alert` only. `magic_link` is refused **by name**: a fresh link is a live credential with a new expiry, and re-issuing one because an old send failed hands out a login by way of an error log. `outreach` is refused because the cadence owns its own steps and already considers them spent. Everything else is **shown** with its full error rather than rebuilt from context the ledger does not keep. The resend re-reads the worklist **server-side** rather than trusting the browser's copy, is admin-gated twice (needsAuth + `requireAdmin`), and hands the page only masked addresses. `scripts/test-email-resend.mjs` (29 assertions, in `build:check`) pins all three rules and the wiring; **verified against the unfixed code first: 27 of the 29 fail.** Route existence and the admin gate confirmed against a booted server (401 unauthenticated on both). **STILL OPEN, named rather than left silent:** (1) a dead transport still burns a cadence step — `sendStepForBooking` marks `steps[n]` BEFORE sending (deliberate, for crash safety) and the sweep skips a marked step, so an auth outage consumes follow-ups that never went out; (2) `lead_alert` and `outreach` are **not** in `CUSTOMER_FACING`, so the 50 outreach failures paged nobody; (3) the whole system still hangs on one Gmail app password, which caps at 500/day and dies whenever the Google account password changes — a transactional provider with an API key is the durable fix. **Patrick's acceptance test — not yet walked:** (1) with the new app password live, open /admin and confirm the Email health panel shows a **Never went out** list; (2) tap **Resend** on one cancellation and confirm that customer receives it; (3) reload and confirm that row is gone; (4) confirm a `magic_link` or `outreach` row shows **send by hand** and offers no button; (5) confirm the masked addresses in the panel are still masked. |
| FLOW-33 | **The customer answers the blast — and Patrick can see what they said** — **UNMAPPED** (opened 2026-09-11, the morning after the first assignment blast) | Hop chain: **`/a/<token>` cancel panel → `POST /api/appointment/<token>/cancel` → `appointmentActions.cancel()` → `bookings.cancel()` → `removalCode` on the record → Patrick's email + SMS alert**, and separately **`bookings.list()` → `withCustomerState()` → `GET /api/bookings` → the bookings page badge**. Two symptoms, one defect: the system knew things about the customer and showed them to nobody. **BLAST-01 (found + fixed 2026-09-11):** cancellations came back with no reason at all. The panel had a free-text box labelled "Anything we should know? (optional)" and *not one customer typed in it* — Patrick: "People came back canceled. We need to prompt them to request why?" A blank column can't tell "I already had it done" (nothing to do) from "I'm using another company" (a lost customer) from "I'm selling" (take the property off next season's list), and those call for opposite actions next February. Fixed with a five-item tap-able list defined **once** in `lib/bookings.js` (`CUSTOMER_CANCEL_REASONS`: already done / another company / selling / not this year / something else), sent to the page on the appointment summary so the page never keeps its own copy of the words, written to the **same `removalCode` field** the tech's own removal vocabulary uses so next February is one query and not two half-answers, and echoed in both the email and the SMS alert — the SMS subject now carries the reason, because that is the one Patrick reads first. "Yes, cancel it" starts **disabled** until a reason is picked ("Something else" is the escape hatch, so nobody is trapped by the list), and the panel offers **"Pick a different day instead"** above the reasons for the customer whose *day* is the problem rather than the service. **BLAST-02 (found + fixed in the same change, separate defect):** the bookings page showed every assigned customer as **Confirmed** from the moment Patrick booked them, so after the blast there was no way to see who had actually replied — Patrick: "i'd like to see it maybe just say 'sent' first, and then once the customer clicks confirm then it flicks over to confirm." **The obvious fix would have broken the campaign.** Adding a `sent` *status* would have stopped follow-up steps 2-6 dead for all 67 customers, silently, because `cadenceBookings` selects `status === "confirmed"` — and `confirmed` is also what `holdsItsSlot()` reads to keep the truck's slot on the calendar. The booking **is** confirmed; what was missing is that the customer's acknowledgement is a *different question*. `customerState()` in `lib/bookings.js` derives it from the outreach record already on the booking (Assigned → Sent → Confirmed / Moved / Time requested / Any time; a self-booked appointment reads Booked because they said yes by booking; any dead status wins outright), the server stamps it onto every record via `withCustomerState()`, and the page renders the stamped label. **No migration and no backfill:** the 67 customers already messaged and not yet answered started reading "Sent" the moment it deployed, which is what "retract whats already been done" asked for. A **Sent** filter was added beside Confirmed; a sent booking matches both, which is the truth. **Deliberately left alone, named here rather than left silent:** the booking DETAIL page still shows the raw engine status in its summary chip, because it sits directly above the editable status control and the two must agree. `scripts/test-customer-state.mjs` (56 assertions, in `build:check`) pins every state word, that `status` is untouched and the cadence still selects on it, that the page has no second copy of either vocabulary, and — the sharp edge — that a customer's phone **cannot** post the tech code `no_answer` and have `bookings.cancel()` resolve its own booking to a chargeable `no_show`. **Verified against the unfixed code first: 45 of the 56 fail.** **Patrick's acceptance test — not yet walked:** (1) open a real appointment link and tap Cancel — confirm the reason list appears and "Yes, cancel it" stays greyed out until you pick one; (2) tap "Pick a different day instead" and confirm it opens the day picker; (3) cancel with a reason and confirm the email and text you receive both name it; (4) open /admin/bookings and confirm customers who were messaged but haven't replied read **Sent**, and one who tapped confirm reads **Confirmed**; (5) confirm the Sent filter lists exactly those. **Touches FLOW-02 and the assignment cadence** — re-verify a follow-up step still sends after this deploys. |
