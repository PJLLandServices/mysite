...
**Related gap — found 2026-08-19, NOT fixed, needs a decision.** The **customer portal** has no route into booking at all. Its `service_complete` state tells the customer *"Reports and invoices for every visit are in your Service History below. Book again any time."* and the page contains no link to `book.html` — not seasonal, not generic. An existing customer who logs in to re-book has nothing to tap. Fixing it means deciding what the CTA books: the customer portal is **lead-scoped** while the seasonal tier depends on a **property's** zone count, so a customer with more than one property has no single right answer. Options: a generic "Book a service →" to `book.html` (safe, loses the tier handoff); or a per-property CTA on each Service History / property card reusing `begin-booking` (keeps the express path, needs the portal to expose property tokens). Touches **FLOW-02 (PASS)**, so it needs re-verification whichever way it goes. |
