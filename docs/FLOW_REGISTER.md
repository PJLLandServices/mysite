...
| CRM-05 | Low — **OPEN, report delivered, deletion NOT yet done** | Two SEO-spam submissions via contact form. **Register premise was out of date:** the intake gate (`server/lib/anti-bot.js`, ahead of any disk write on `/api/quotes`) is five layers — honeypot, time-trap, per-IP rate limit, email normalization, and Turnstile (worth confirming `TURNSTILE_SECRET_KEY` is set on Render) — with rejections logged to `bot-blocked.log`. **None of it stops a human typing an SEO pitch at human speed, which is what these two were.** Recommendation (JOB-009, awaiting Patrick's decision): content heuristics that **flag, don't block** — reuse the scanner's vocabulary at intake to set the lead schema's existing `botFlagged`, so suspect leads arrive pre-flagged instead of posing as leads. No customer is ever blocked. Stricter Turnstile levels taxes every real customer and still doesn't stop humans — not recommended. **No CAPTCHA or gating added** per the standing instruction. At two submissions total the honest answer is option 1 or nothing. |
...
| MISC-01 | **ANSWERED — they do not 404. Not a broken link.** Fixed in JOB-009; acceptance tap pending. All four pages exist in the repo, are referenced by **84 pages** each (the sitewide footer), appear in `sitemap.xml`, and return **200** when requested from a locally booted server. The only real gap was that **`sitemap.html`** — the human-readable Site Map page, *not* `sitemap.xml` — omitted them from Service Areas; they have been added (footer ordering, descriptions from each page's own meta description). *Caveat:* outbound to the live host is blocked from the build sandbox, so the 200s are local, not production — Patrick's four footer taps remain the production confirmation. **`sitemap.xml` was never part of this defect** and has carried all 18 city URLs since ccf7604 (2026-07-15); it has no server route and ships as a static file. |
