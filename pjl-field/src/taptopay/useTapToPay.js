...
export function useTapToPay({ onProgress } = {}) {
  ...
  // One payment, against the charge the SERVER created for this invoice
  // (api.startTerminalPayment). The phone never names an amount: the
  // server charges the invoice's balance, so a re-keyed or stale number
  // cannot be taken. The reader collects and confirms; whether the invoice
  // is paid is then the server's call, from Stripe (api.finalizeTerminalPayment).
  //
  ...
  const collect = useCallback(async ({ clientSecret }) => {
    if (!clientSecret) {
      return { ok: false, outcome: 'declined', error: 'The server did not start a payment.' };
    }
    setError(null);
    ...
    try {
      ...
      if (retrieveError || !paymentIntent) {
        throw new Error(retrieveError?.message || 'Could not start the payment.');
      }
      ...
      if (collectError) {
        const canceled = /cancel/i.test(`${collectError.code || ''} ${collectError.message || ''}`);
        return {
          ok: false,
      ...
      if (confirmError) {
        return {
          ok: false,
          outcome: /timed?.?out/i.test(`${confirmError.code || ''} ${confirmError.message || ''}`) ? 'timed_out' : 'declined',
          error: confirmError.message || 'The payment was declined.',
          paymentIntentId: collected?.id || paymentIntent.id || null,
        };
      }
      return {
        ok: true,
        outcome: 'approved',
        paymentIntentId: confirmed?.id || collected?.id || paymentIntent.id || null,
      };
    } catch (err) {
