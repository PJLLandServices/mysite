...
try {
  ...
  let r = await srv.qpatch(id, { techNotes: "Customer mentioned the side gate sticks." });
  ...
  r = await srv.qpatch(id, { zones: relabelled });
  ...
  r = await srv.qpatch(id, { zones: five });
  ...
  r = await srv.qpatch(id, { zones: four });
  ...
  const frozen = await srv.qpatch(id, { onSiteQuote: { ...wo.onSiteQuote, builderLineItems: [] } });
  ...
  // ---- F. before completion, with a bypass --------------------------------
  {
    ...
    const more = await srv.qpatch(gid, { zones: [...cur, { number: 5, location: "Side yard", status: "ok", kind: "zone" }] });
