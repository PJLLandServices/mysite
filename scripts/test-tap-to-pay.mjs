...
//
// Ported 2026-09-24 from the unmerged claude/pjl-field-taptopay branch onto
// current code, with one deliberate change: the phone no longer creates the
// PaymentIntent or marks the invoice paid. The server does both
// (scripts/test-taptopay-server.mjs); this file pins that the app only
// collects against the server's charge and then asks the server to confirm.
//
// Run: node scripts/test-tap-to-pay.mjs   (build:check on the Tap to Pay branch)
//
...
const serverStripe = read('server/lib/stripe.js');
...
check('the SDK is a real dependency at a pinned version', () => {
  ...
  // EXACT, no caret: beta.32 is the version that ran on Patrick's phone in
  // September. A caret would let npm take a beta published yesterday into
  // the one build Apple reviews.
  assert.match(v, /^0\.0\.1-beta\.\d+$/, `not pinned exactly: ${v}`);
});

// ---- The build: Xcode on the Mac, not EAS -------------------------------
//
// EAS cloud builds can only make ad-hoc / App Store profiles, and Apple's
// development entitlement only rides a development profile
// (docs/TAP_TO_PAY.md), so there is deliberately no EAS profile for this
// build. What must hold is that the everyday production profile is untouched.
...
check('5.2: the Tap to Pay button comes before the other payment actions', () => {
  const tapAt = invoice.indexOf('accessibilityLabel={tap.label}');
  const linkAt = invoice.indexOf('>Take payment now</Text>');
  const recordAt = invoice.indexOf('>Record a payment I took</Text>');
...
check('CA: Interac is an accepted method, not just credit', () => {
  ...
  // debit. Omitting it declines those cards at the tap. The SERVER creates
  // the intent now, so that is where the method list lives.
  const at = serverStripe.indexOf('async function createTerminalPaymentIntent');
  assert.ok(at > 0, 'the server does not create Tap to Pay intents');
  const body = serverStripe.slice(at, at + 1200);
  assert.match(body, /"interac_present"/, 'Interac debit would be declined at the tap');
  assert.match(body, /"card_present"/);
});
...
check('the phone never names the amount — the server charges the balance', () => {
  // The reference branch computed cents on the phone and created the intent
  // there. Now the phone asks the server to start a charge and sends nothing.
  assert.ok(!/createPaymentIntent\(/.test(hook), 'the phone is creating its own PaymentIntent again');
  assert.match(hook, /retrievePaymentIntent\(clientSecret\)/, 'the reader does not collect against the server\'s intent');
  assert.match(api, /terminal-intent`, 'POST', \{\}/, 'startTerminalPayment sends a body — the server must be the only source of the amount');
  const at = invoice.indexOf('const tapToPay');
  const block = invoice.slice(at, invoice.indexOf('\n  };\n', at));
  assert.match(block, /startTerminalPayment\(invoiceId\)/, 'the tap path does not ask the server to start the charge');
  assert.ok(!/TextInput|setAmount\(|amountCents|balanceDue/.test(block), 'the tap path reads an amount of its own');
});

check('the invoice reads Paid only when the server has checked with Stripe', () => {
  // The reference branch POSTed /payments with the intent id in a note —
  // the phone's word as the evidence. Now: finalize, which re-reads the
  // intent from Stripe on the server.
  const at = invoice.indexOf('const finalizeTap');
  assert.ok(at > 0, 'there is no finalize step after an approved tap');
  const tapBlock = invoice.slice(at, invoice.indexOf('// Money that arrived some other way'));
  assert.match(tapBlock, /finalizeTerminalPayment\(invoiceId, paymentIntentId\)/, 'an approval is not confirmed with the server');
  assert.ok(!/recordInvoicePayment/.test(tapBlock), 'the tap path records its own payment instead of letting the server verify it');
  // An approval the server could not confirm yet must never invite a
  // second charge.
  assert.match(invoice, /approved_unconfirmed/, 'an approved-but-unconfirmed tap is not its own state');
  assert.match(invoice, /do not take the\s+payment again/, 'the unconfirmed state does not warn against charging twice');
});

check('one location permission string, covering attendance AND payments', () => {
  // The Stripe plugin OVERWRITES NSLocationWhenInUseUsageDescription with
  // its own prop (read from its source), so the attendance wording that
  // expo-location relies on silently vanished in the reference build.
  const plist = appJson.expo.ios.infoPlist.NSLocationWhenInUseUsageDescription;
  const stripePlugin = appJson.expo.plugins.find((p) => Array.isArray(p) && p[0] === '@stripe/stripe-terminal-react-native');
  assert.ok(stripePlugin, 'the Stripe Terminal config plugin is not listed');
  assert.equal(stripePlugin[1].locationWhenInUsePermission, plist, 'the plugin and infoPlist disagree — the plugin wins and one purpose disappears');
  assert.match(plist, /attendance/, 'the attendance purpose is gone');
  assert.match(plist, /payment/, 'the payment purpose is missing');
});
