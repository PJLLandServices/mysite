...
//
// Usage (one WO per run — 11 strandeds means 11 deliberate runs):
...
async function main() {
  ...
  if (!wo.propertyId) {
    console.error("  REFUSED: no propertyId — the cascade cannot fire (no service record, no invoice, no warranty). Link the WO to its property in the CRM first.");
    process.exit(1);
  }
