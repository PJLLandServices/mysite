...
- **View an individual invoice** (read-only) via the SMS deep-link at `/portal/invoice/:id?t=<portalToken>` — line items, totals, status, and a "Pay this invoice" button. Mirrors the formal invoice without exposing internal notes or audit trail.
  - **The Pay button deep-links to PJL's own payment page** at `/pay/invoice/:id?t=<paymentToken>`, in the same tab. It does **not** open a QuickBooks-hosted payment page in a new tab — that was never how it was built, and this line said otherwise until Jul 2026. The page is PJL-branded and PJL-served; only the card tokenization call goes to Intuit, direct from the customer's browser. `paymentToken` is a separate secret from `portalToken` so the two surfaces revoke independently. See `SYSTEM_OVERVIEW.md` → **Payments surface** for the endpoints, the `card.address` AVS payload, and the `paymentAttempts[]` log.
...
22. **A capture is not idempotent.** Every other recovery path in this system is safe to re-run — cascades are idempotent, QB pushes re-resolve, the SMS sweep re-checks, a failed sync retries. **A card charge is not.** It moves money on the first call and again on the second. So: no automatic retry, no backoff loop, no "try once more before showing the error" anywhere in the charge path, and no idempotency key reused across attempts (`Request-Id` in `quickbooks.chargeCard` is random per call *deliberately*, so a customer's deliberate re-submit is a genuinely new charge rather than a silently deduped no-op). A failed attempt records to `invoice.paymentAttempts[]` and **stops**; the customer decides whether to try again. The corollary is that the failure must be legible enough for them to decide — which is why the Intuit error code is mapped to plain language and why the raw gateway string is logged instead of displayed. QB Payments AVS brief, Jul 2026.

23. **Card data never reaches PJL's server.** PAN, CVC and expiry go from the customer's browser **directly** to `api.intuit.com` (no `Authorization` header — the tokenization endpoint accepts unauthenticated POSTs by design, same model as Stripe.js); the Node process only ever handles the resulting opaque token. This is what keeps the integration in PCI **SAQ-A-EP** instead of SAQ-D, a materially heavier compliance burden. No change may route card data through `pjllandservices.com` — not "temporarily for debugging", not through a log line, not through an error message. The `paymentAttempts[]` writer is an explicit field allowlist for exactly this reason: card data cannot be persisted even by mistake. **Billing address fields are NOT card data** and are safe server-side — collecting a billing street address is how a card-not-present charge passes AVS, and its absence is what made Amex charges fail while Visa and Mastercard cleared. This rule was in force in the code from PR 3 but had never been written down; the AVS brief cited it as "r16", which is a different rule entirely (self-service portal enforcement). QB Payments AVS brief, Jul 2026.
